...
import HawkswapFactory from '@hawk-protocol/hawk-swap-core/build/HawkswapFactory.json'
import IHawkswapPair from '@hawk-protocol/hawk-swap-core/build/IHawkswapPair.json'
...
import HawkswapRouter from '../../build/HawkswapRouter.json'
...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  // deploy V2
  const factoryV2 = await deployContract(wallet, HawkswapFactory, [wallet.address])

  // deploy routers
  const router01 = await deployContract(wallet, UniswapV2Router01, [factoryV2.address, WETH.address], overrides)
  const router02 = await deployContract(wallet, HawkswapRouter, [factoryV2.address, WETH.address], overrides)
  ...
  const pair = new Contract(pairAddress, JSON.stringify(IHawkswapPair.abi), provider).connect(wallet)
  ...
  const WETHPair = new Contract(WETHPairAddress, JSON.stringify(IHawkswapPair.abi), provider).connect(wallet)
