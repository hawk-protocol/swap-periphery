...
library MdexOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IHawkswapPair(pair).price0CumulativeLast();
        price1Cumulative = IHawkswapPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IHawkswapPair(pair).getReserves();
...
contract Oracle {
    ...
    function update(address tokenA, address tokenB) external {
        address pair = HawkswapLibrary.pairFor(address(factory), tokenA, tokenB);
    ...
    function consult(address tokenIn, uint amountIn, address tokenOut) external view returns (uint amountOut) {
        address pair = HawkswapLibrary.pairFor(address(factory), tokenIn, tokenOut);
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'HawkSwapFactory: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'HawkSwapFactory: ZERO_ADDRESS');
    }
}
